...
//BUGS RECTIFIEDD       
// 1. Fixed the logic for handling the case when there are no elements in the set.
// 2. Corrected the logic for calculating the best value in the treap.  
...
struct Node {
    ...
    Node(int k, int _w, unsigned int _pr)
        : key(k), w(_w), pr(_pr), l(nullptr), r(nullptr),
...
Node* insert(Node* t, Node* n) {
    ...
    if (n->key < t->key) t->l = insert(t->l, n);
...
Node* erase_key(Node* t, int key) {
    ...
    if (key < t->key) t->l = erase_key(t->l, key);
    else t->r = erase_key(t->r, key);
    pull(t); return t;
}
...
int main() {
    ...
    while (T--) {
        ...
        for (int v = 1; v <= n; ++v) {
            ...
            if (pos[v].empty()) cur[v] = 0;
            else if (pos[v].size() == 1) cur[v] = 1;
            else {
                long long bw = root[v] ? root[v]->best : NEG;
                cur[v] = (int)(1 + max(bw, 0LL));
            }

            if (cur[v] > 0) best.insert(cur[v]);
        }
        ...
        while (q--) {
            ...
            if (old != x) {
                int v = old;
                if (cur[v] > 0) {
                    auto it = best.find(cur[v]);
                    if (it != best.end()) best.erase(it);
                }

                {
                    ...
                    auto itNext = next(it);
                    if (itPrev != pos[v].end()) root[v] = erase_key(root[v], *itPrev);
                    if (itNext != pos[v].end()) root[v] = erase_key(root[v], idx);
                ...
                if (pos[v].empty()) cur[v] = 0;
                else if (pos[v].size() == 1) { cur[v] = 1; root[v] = nullptr; }
                else {
                    long long bw = root[v] ? root[v]->best : NEG;
                    cur[v] = (int)(1 + max(bw, 0LL));
                }
                ...
                if (pos[v].size() == 1) { cur[v] = 1; root[v] = nullptr; }
                else {
                    long long bw = root[v] ? root[v]->best : NEG;
                    cur[v] = (int)(1 + max(bw, 0LL));
                }
